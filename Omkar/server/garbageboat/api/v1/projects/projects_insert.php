...
require("../database/dbh.inc.php");
	...
	if(isset($_POST["serial_id"]) && isset($_POST["project_name"]) && isset($_POST["project_description"]) && isset($_POST["location_lat"]) && isset($_POST["location_lng"]) && isset($_POST["is_anonymous"])){
		...
		if(empty($serial_id) || empty($project_name)  || empty($project_description) || empty($location_lat) || empty($location_lng) || !is_numeric($is_anonymous)){
			$message["error"] = true;
			$message["error_message"] = "Require all parameters ";	
			...
			$stmt->bind_param("isssss", $serial_id, $project_name, $project_description, $location_lat, $location_lng, $is_anonymous);
			$stmt->execute();
			$stmt->close();
			$message["error"] = false;
