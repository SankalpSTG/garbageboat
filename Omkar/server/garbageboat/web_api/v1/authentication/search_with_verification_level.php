...
require("../database/dbh.inc.php");
	...
	if(isset($_POST["search"])){
		...
		if(empty($search)){
			...
			$stmt = $conn->prepare("SELECT cred.serial_id, cred.email_id, cred.mobile_no, cred.verification_level, usr. company_name, usr.logo_url, usr.address, usr.pincode FROM credentials as cred, users as usr WHERE cred.account_level = 0 and cred.verification_level = 2 and usr.credential_id = cred.serial_id LIKE ? limit 10");
