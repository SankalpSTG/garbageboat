<?php
require("../database/dbh.inc.php");

	$message["error"] = true;
	$message["error_message"] = "Require all parameters";
	if(isset($_POST["query"])){
		$query = mysqli_real_escape_string($conn, $_POST["query"]);
		if(empty($query)){
			$message["error"] = true;
			$message["error_message"] = "Require all parameters";	
			die(json_encode($message));
		}else{
			$result = mysqli_query($conn, "SELECT cred.serial_id, cred.email_id, cred.mobile_no, cred.account_level, cred.verification_level, usr.company_name, usr.logo_url, usr.address, usr.pincode FROM credentials as cred, users as usr WHERE cred.account_level = -1 AND usr.credential_id = cred.serial_id AND (cred.email_id LIKE '%$query%' OR cred.mobile_no LIKE '%$query%' OR usr.company_name LIKE '%$query%' OR usr.address LIKE '%$query%') limit 10");
			if(mysqli_num_rows($result) > 0){
				$data = [];
				while ($row = mysqli_fetch_assoc($result)){
				$responce = [];
				$responce["serial_id"] = (int)$row["serial_id"];
				$responce["email_id"] = (string)$row["email_id"];
				$responce["mobile_no"] = (int)$row["mobile_no"];
				$responce["account_level"] = (int)$row["account_level"];
				$responce["verification_level"] = (int)$row["verification_level"];
				$responce["company_name"] = (string)$row["company_name"];
				$responce["logo_url"] = (string)$row["logo_url"];
				$responce["pincode"] = (int)$row["pincode"];
				$responce["address"] = (string)$row["address"];
				array_push($data, $responce);
				}
				$message["data"] = $data;
				$message["error"] = false;
				$message["error_message"] = "Success";	
				die(json_encode($message));
			}else{
				$message["error"] = true;
				$message["error_message"] = "No user found";	
				die(json_encode($message));	
			}
		}
	}die(json_encode($message));
?>